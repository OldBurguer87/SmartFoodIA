...
def map_order(order: Order, integration: StoreIntegration) -> dict:
    ...
    method={'method':order.payment_method,'type':effective_payment_type,'currency':'BRL','value':float(order.total),'prepaid':effective_payment_type=='PREPAID','cash':({'changeFor':float(order.change_for)} if order.payment_method=='CASH' and order.change_for is not None else None),'card':None,'wallet':None}
