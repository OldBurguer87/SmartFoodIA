...
class ConsumerPartnerService:
    def __init__(self, adapter=None, authenticator=None):
        self.adapter=adapter or ConsumerPartnerAdapter(); self.authenticator=authenticator or ConsumerAuthenticator()
    ...
    def update_status(self, db: Session, *, store: Store, payload):
        ...
        if changed and payload.justification: phrase += f': {payload.justification}'
